...
12. **通知配置**：`scripts/config.sh`（已 gitignore，含 iMessage 接收人 `NOTIFY_TO`；模板见 `scripts/config.example.sh`）；iMessage 发送走本机共享通道 `~/projects/tools/macos-notify/send-imessage.sh`，项目内不再保留发送脚本
...
- **信源独立性与口径归属**：独立信源 = 编辑上独立于报道对象（官方公告+其转载只算一个）；仅官方来源的性能/规模声明必须带"官方称/公司披露"字样；仅官方口径的 S 级事件最高进核心动态（可置顶并标注"官方口径"），头条须至少一个独立信源；海外事件链英文原始来源，中文翻译稿只作佐证。
...
- 分类使用三级标题：`### 🔥 头条`、`### 📌 核心动态`、`### 📎 其他要闻`、`### 💡 今日洞察`
- 新闻标题加粗 `**标题**`
- **每条新闻必须附带可点击的来源链接**：`- **来源**：[来源名](URL) | **时间**：X月X日`；多个来源时用 `/` 分隔多个链接
- **条目格式（头条/核心动态）**：标题为独立加粗行并带 Emoji 前缀，下接来源行、详情段落（补充数字/背景/与往日报道的呼应）与"信号："分析行，条目间用 `---` 分隔；其他要闻为单行条目（标题内嵌来源链接+一句点评）。禁止只写一句概述的简略风格
